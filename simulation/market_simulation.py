...
class MarketSession:
    def __init__(self):
        ...
        self.is_active = False  # Changed to False by default
        self.tick_count = 0
    ...
    def initialize_session(self):
        """Initialize market state for new session"""
        from simulation import market_state
        # Reset trading volumes
        for stock in market_state.trading_volume:
            market_state.trading_volume[stock] = 0
        
        # Ensure all teams have valid portfolios
        for team_id in range(market_state.TEAM_COUNT):
            if team_id not in market_state.team_portfolios:
                market_state.team_portfolios[team_id] = {
                    'cash': market_state.STARTING_BUDGET,
                    'holdings': {},
                    'transactions': [],
                    'holdings_value': 0,
                    'total_value': market_state.STARTING_BUDGET
                }
    ...
    @safe_operation
    def update(self):
        """Perform periodic market updates."""
        if not self.session_active or not self.is_active or self.pause_lock:
            return
...
# Create global market session
market_session = MarketSession()
market_session.is_active = False
market_session.session_active = False
